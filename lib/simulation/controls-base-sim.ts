...
export class ControlsBaseSim {
  ...
  configureMotor(options: ControlsBaseSimOptions): void {
    ...
    try {
      ...
      console.log(
        `Configured ${motorCount} ${motorType} motor(s) with gearing ${gearing}:1`,
        {
      ...
      console.warn(
        `Motor type ${motorType} not found in configuration, using NEO as fallback`,
      );
  ...
  calculateFeedforward(velocity: number, acceleration: number): number {
    ...
    const gravityComponent =
      (this.kG || 0) * Math.cos(this.arm ? this.position : 0);
