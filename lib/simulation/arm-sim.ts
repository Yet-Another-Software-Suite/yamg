...
export class ArmSim extends ControlsBaseSim {
  ...
  drawAngleMarkers(ctx: CanvasRenderingContext2D, x: number, y: number): void {
    ...
    ctx.beginPath()
    ctx.moveTo(x, y)
    ctx.lineTo(x + radius * Math.cos(this.position), y - radius * Math.cos(this.position))
    ctx.stroke()
    ...
    ctx.beginPath()
    ctx.moveTo(x, y)
    ctx.lineTo(x + radius * Math.cos(this.target), y - radius * Math.cos(this.target))
    ctx.stroke()
  }
  ...
  drawTelemetry(ctx: CanvasRenderingContext2D): void {
    ...
    // Draw telemetry data
    ctx.fillText(`Angle: ${((this.position * 180) / Math.PI).toFixed(1)}°`, textX, textY)
    textY += lineHeight
    ctx.fillText(`Target: ${((this.target * 180) / Math.PI).toFixed(1)}°`, textX, textY)
    textY += lineHeight
    ctx.fillText(`Velocity: ${((this.velocity * 180) / Math.PI).toFixed(1)}°/s`, textX, textY)
    textY += lineHeight
