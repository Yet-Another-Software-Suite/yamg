...
async function processTemplate(
  ...
  const templateData = {
    ...
    enableSoftLimits: !!(
      data.softLimits?.forward !== undefined ||
      data.softLimits?.reverse !== undefined
    ),
    forwardSoftLimit:
      data.softLimits?.forward !== undefined ? data.softLimits.forward : null,
    reverseSoftLimit:
      data.softLimits?.reverse !== undefined ? data.softLimits.reverse : null,
    statorCurrentLimit: data.currentLimits?.stator || 40,
    supplyCurrentLimit: data.currentLimits?.supply || 40,
    enableStatorLimit: !!(
      data.currentLimits?.stator && data.currentLimits.stator > 0
    ),
    enableSupplyLimit: !!(
  ...
  if (data.mechanismType === "Elevator" && data.elevatorParams) {
    const startingHeight =
      data.elevatorParams.startingHeight !== undefined
        ? data.elevatorParams.startingHeight
        : 0.0;
    const hardLimitMin =
      data.elevatorParams.hardLimitMin !== undefined
        ? data.elevatorParams.hardLimitMin
        : 0.0;
    const hardLimitMax =
      data.elevatorParams.hardLimitMax !== undefined
        ? data.elevatorParams.hardLimitMax
        : 1.0;
    const drumRadius =
      data.elevatorParams.drumRadius !== undefined
  ...
  if (data.mechanismType === "Arm" && data.armParams) {
    const length =
      data.armParams.length !== undefined ? data.armParams.length : 1.0;
    const startingPosition =
      data.armParams.startingPosition !== undefined
        ? data.armParams.startingPosition
        : 0.0;
    const hardLimitMin =
      data.armParams.hardLimitMin !== undefined
        ? data.armParams.hardLimitMin
        : 0.0;
    const hardLimitMax =
      data.armParams.hardLimitMax !== undefined
        ? data.armParams.hardLimitMax
        : 90.0;
  ...
  try {
    ...
    const actualTemplateName =
      templateName === "mechanism-subsystem"
        ? mechanism.templateName
        : templateName;
...
function processMotorControllerTemplate(
  data: FormValues,
): Record<string, string> {
  try {
    const motorControllerModule = getMotorControllerModule(
      data.motorControllerType,
    );
    ...
    return {
      ...
      motorControllerSimulationPeriodic:
        motorControllerModule.getSimulationPeriodic(),
